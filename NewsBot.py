...
class NewsBot(telebot.async_telebot.AsyncTeleBot):
    ...
    async def start_command(self, message: telebot.async_telebot.types.Message):
        ...
        button = telebot.async_telebot.types.KeyboardButton("/set_sorces")
        button2 = telebot.async_telebot.types.KeyboardButton("/set_category")
        button3 = telebot.async_telebot.types.KeyboardButton("/set_q")
        button4 = telebot.async_telebot.types.KeyboardButton("/check_news")
...
3. Ключевое слово: любое слово на вашем языке. По умолчанию без ключевого слова.""",
        ...
        await self.send_message(
            message.chat.id,
            "Чтобы узнать настройки параметров, выберите команду CheckNews, чтобы получить новости, выберите команду GetNews",
        )
...
3. Ключевое слово: {self.list_of_data[message.chat.id]["q"] if self.list_of_data[message.chat.id]["q"] is not None else "Без ключевого слова"}""",
    ...
    async def get_news(self, message: telebot.types.Message):
        ...
        if len(self.list_of_data[message.chat.id]["top_headlines"]["results"]) == 0:
            ...
            await self.send_message(
                message.chat.id,
                "Если вы хотите получить больше новостей, то попробуйте другой запрос, или подождите некоторое время, когда новости по вашим параметрам появятся",
                reply_markup=markup,
            )
    ...
    async def set_category(self, message: telebot.types.Message):
        ...
        await self.send_message(
            message.chat.id, "Выбирите категорию", reply_markup=markup
        )

    async def set_category2(self, message: telebot.types.Message):
        if message.text in self.list_of_categorys:
            self.list_of_data[message.chat.id]["category"] = self.list_of_categorys[
                message.text
            ]
            markup = telebot.types.ReplyKeyboardMarkup(resize_keyboard=True)
            button = telebot.async_telebot.types.KeyboardButton("/set_sorces")
    ...
    async def run(self):
        ...
        self.register_message_handler(self.check_news, commands=["check_news"])
        self.register_message_handler(self.get_news, commands=["get_news"])
        self.register_message_handler(self.set_sorces, commands=["set_sorces"])
        ...
        self.register_message_handler(self.set_q, commands=["set_q"])
        self.register_message_handler(
            self.set_q2,
            func=lambda message: True
